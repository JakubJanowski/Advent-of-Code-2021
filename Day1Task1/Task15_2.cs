...
namespace Tasks {
    internal class Task15_2: ITask {
        ...
        public string Solve(string[] data) {
            byte[][] initialMap = data.Select(row => row.Select(digit => (byte)(digit - '0')).ToArray()).ToArray();
            byte[][] map = new byte[initialMap.Length * 5][];
            for (int y = 0; y < map.Length; y++) {
                int originalY = y % initialMap.Length;
                map[y] = new byte[initialMap[y % initialMap.Length].Length * 5];
                for (int x = 0; x < map[y].Length; x++) {
                    int originalX = x % initialMap[originalY].Length;
                    map[y][x] = (byte)((initialMap[originalY][originalX] + y / initialMap.Length + x / initialMap[originalY].Length - 1) % 9 + 1);
                }
            }
