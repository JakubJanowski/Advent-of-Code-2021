    ...
    internal class Task19_2: ITask {
        ...
        public string Solve(string[] data) {
            ...
            for (int i = 0; i < scanners.Count - 1; i++) {
                Point p1 = scanners[i].Position!;
                for (int j = i + 1; j < scanners.Count; j++) {
                    Point p2 = scanners[j].Position!;
                    maxDistance = Math.Max(maxDistance, Math.Abs(p1.X - p2.X) + Math.Abs(p1.Y - p2.Y) + Math.Abs(p1.Z - p2.Z));
                }
            }
